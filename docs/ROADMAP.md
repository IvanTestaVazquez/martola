...
\- Creación da táboa \`plant\_species\`.
\- Creación da táboa \`garden\_plants\`.
\- Introdución de claves foráneas.
\- Activación da integridade referencial mediante \`PRAGMA foreign\_keys = ON\`.
...
- Creación do modelo `PlantEvolutionRecord`.
- Evolución do esquema SQLite á versión 3.
- Implementación da migración v2 → v3.
- Creación da táboa `plant_evolution_records`.
- Relación `GardenPlant 1:N PlantEvolutionRecord`.
- `ON DELETE CASCADE` entre `garden_plants` e `plant_evolution_records`.
- Creación de `PlantEvolutionRecordRepository`.
- Creación de `SQLitePlantEvolutionRecordRepository`.
- Verificación do CRUD de rexistros de evolución contra SQLite.
- Creación de `PlantEvolutionViewModel`.
- Integración de `PlantEvolutionViewModel` mediante `MultiProvider`.
- Mantemento da planta activa mediante `_currentPlantId`.
- Creación de `PlantEvolutionListScreen`.
- Creación de `AddPlantEvolutionRecordScreen`.
- Creación de `PlantEvolutionDetailsScreen`.
- Creación de `EditPlantEvolutionRecordScreen`.
- CRUD completo do módulo de evolución integrado con SQLite e Provider.
...
\- [x] Engadir \`sqflite\`.
\- [x] Engadir \`sqflite\_common\_ffi\`.
\- [x] Engadir \`path\`.
\- [x] Engadir \`path\_provider\`.
...
\- [x] Crear a táboa \`plant\_species\`.
\- [x] Crear a táboa \`garden\_plants\`.
\- [x] Introducir claves foráneas.
\- [x] Activar a integridade referencial mediante \`PRAGMA foreign\_keys = ON\`.
...
- [x] Evolucionar o esquema á versión 3.
- [x] Crear a táboa `plant_evolution_records`.
- [x] Implementar a migración SQLite v2 → v3.
- [x] Conservar os datos existentes durante a migración v2 → v3.
- [x] Crear a relación entre `garden_plants` e `plant_evolution_records`.
- [x] Configurar `ON DELETE CASCADE` para eliminar os rexistros dunha planta cando esta se elimina.
- [x] Crear `SQLitePlantEvolutionRecordRepository`.
- [x] Implementar o CRUD SQLite de rexistros de evolución.
- [x] Consultar os rexistros de evolución mediante `plantId`.
...
A infraestrutura SQLite está operativa e o esquema evolucionou á versión 3.
...
\- [x] Crear táboa \`plant\_species\`.
\- [x] Crear táboa \`garden\_plants\`.
...
\- [x] Implementar CRUD SQLite de especies.
\- [x] Implementar CRUD SQLite de plantas.
\- [x] Implementar consulta de plantas mediante \`gardenId\`.

**### State Management**

\- [x] Crear \`PlantSpeciesViewModel\`.
\- [x] Crear \`PlantsViewModel\`.
\- [x] Cargar o catálogo de especies mediante \`loadSpecies()\`.
\- [x] Cargar as plantas dunha horta mediante \`loadPlants(gardenId)\`.
\- [x] Manter a horta actualmente cargada mediante \`\_currentGardenId\`.
\- [x] Integrar os ViewModels con \`MultiProvider\`.
\- [x] Sincronizar as Views co estado das plantas mediante \`notifyListeners()\`.
\- [x] Consultar unha planta cargada mediante \`getPlantById()\`.
\- [x] Resolver unha especie cargada mediante \`getSpeciesById()\`.

**### Interface**

\- [x] Crear \`PlantListScreen\`.
\- [x] Mostrar as plantas asociadas a unha horta.
\- [x] Crear formulario de nova planta (\`AddPlantScreen\`).
\- [x] Permitir seleccionar unha especie mediante \`DropdownButtonFormField\`.
\- [x] Permitir seleccionar a data de plantación mediante \`showDatePicker\`.
\- [x] Crear plantas desde a interface.
\- [x] Crear \`PlantDetailsScreen\`.
\- [x] Navegar desde a lista ao detalle dunha planta.
\- [x] Mostrar o nome común da especie no detalle.
\- [x] Crear \`EditPlantScreen\`.
\- [x] Crear o fluxo de edición.
\- [x] Crear o fluxo de eliminación.
\- [x] Solicitar confirmación antes de eliminar unha planta.
\- [x] Actualizar automaticamente lista e detalle tras os cambios.

**## Pending**

\- [ ] Revisar e refinar a interface do módulo.
\- [ ] Valorar ampliacións do catálogo de especies segundo as necesidades do MVP.

**## Deliverable**

Módulo de plantas funcional, persistido mediante SQLite e integrado co módulo de hortas e co catálogo de especies.

**\*\*Estado:\*\*** funcional a nivel de CRUD, persistencia e interface básica. Pendentes refinamentos visuais e futuras ampliacións.
...
**## Session 15 - Plants Repository, State Management and CRUD UI**
...
\- Implementación de \`SQLitePlantSpeciesRepository\`.
\- Implementación do CRUD persistente de especies.
\- Implementación de \`SQLiteGardenPlantRepository\`.
\- Implementación do CRUD persistente de plantas.
\- Consulta das plantas dunha horta mediante \`gardenId\`.
\- Verificación do CRUD de especies e plantas contra SQLite.
\- Verificación do comportamento das claves foráneas durante as operacións reais.
\- Creación de \`PlantSpeciesViewModel\`.
\- Creación de \`PlantsViewModel\`.
\- Integración de \`GardensViewModel\`, \`PlantSpeciesViewModel\` e \`PlantsViewModel\` mediante \`MultiProvider\`.
\- Carga inicial do catálogo mediante \`loadSpecies()\`.
\- Mantemento da horta actualmente cargada mediante \`\_currentGardenId\`.
\- Carga das plantas mediante \`loadPlants(gardenId)\`.
\- Creación dun catálogo inicial local de especies.
\- Creación de \`PlantListScreen\`.
\- Integración da lista de plantas co detalle da horta.
\- Creación de \`AddPlantScreen\`.
\- Selección de especie mediante \`DropdownButtonFormField\`.
\- Selección da data de plantación mediante \`showDatePicker\`.
\- Creación e persistencia de plantas desde a interface.
\- Creación de \`PlantDetailsScreen\`.
\- Navegación desde a lista ao detalle.
\- Resolución do nome común da especie mediante \`PlantSpeciesViewModel\`.
\- Creación de \`EditPlantScreen\`.
\- Edición e persistencia de plantas desde a interface.
\- Eliminación de plantas con diálogo de confirmación.
\- Actualización reactiva das Views mediante Provider.
\- CRUD completo do módulo de plantas desde a interface.

O fluxo funcional actual é:
...
O módulo de plantas queda funcional de extremo a extremo:
...
O seguinte bloque funcional será o módulo de evolución das plantas, que corresponde ao único elemento pendente dentro da definición actual do MVP.
...
Coa finalización do módulo de evolución, o núcleo funcional da definición mínima actual do MVP queda implementado.

As seguintes funcionalidades considéranse ampliacións de alto valor ou fases de consolidación segundo o tempo dispoñible:
